...
#include "proc.h"
#include "defs.h"
#include "sleeplock.h"  //头文件的位置也会影响编译
#include "fs.h"
#include "file.h"
...
void usertrap(void)
{
    ...
    else if (r_scause() == 13 || r_scause() == 15)
    {
        ...
        uint64 *mem;  
        uint64 tmp;                          //分配的内存
        struct vm_area_struct *man = &(p->man); //取得该进程虚拟空间管理结构
        int pte_flag;
        uint64 offset;
        int i;
        ...
        for (i = 0; i < NOFILE; i++)
        {
            if ((uint64)man->mfiles[i].start <= fault_vaddr && (uint64)man->mfiles[i].end > fault_vaddr)
            {
                break;
            }
        }
        if (i == NOFILE)
        {
            printf("usertrap():addr %p not in a file\n", fault_vaddr);
            p->killed = 1;
            exit(-1);
        }
        ...
        memset(mem, 0, PGSIZE);
        if (mappages(p->pagetable, fault_page, PGSIZE, (uint64)mem, pte_flag) != 0)
        {
            kfree(mem);
