...
found:
    ...
    memset(&(p->man), 0, sizeof(struct vm_area_struct));
    return p;
}
...
int fork(void)
{
    ...
    for (i = 0; i < NOFILE; i++) //遍历打开的最大文件数(16)
    {
        if (p->man.mfiles[i].f)          //如果有打开的文件
            filedup(p->man.mfiles[i].f); //则复制一份文件
                                         //ref. xv6_book page 87 create a duplicate reference (filedup)。filedup (kernel/file.c:48) increments the reference count
    }
